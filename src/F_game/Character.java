package F_game;

public class Character {
	String name; //이름
	int maxHp; //최대 체력
	int maxMp; //최대 마력
	int hp; //체력
	int mp; //마나
	int att; //공
	int def; //방
	int level; //렙
	int exp; //경험치
	Item[] items; //템
	
	Character(String name, int hp, int mp, int att, int def){
		this.name = name;
		this.maxHp = hp;
		this.maxMp = hp;
		this.hp = this.maxHp;
		this.mp = this.maxMp;
		this.att = att;
		this.def = def;
		this.level = 1;
		this.exp = 0;
		this.items = new Item[10];
	}
	
	void showinfo(){ //인포창
		System.out.println("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■");
		System.out.println("Name : " + name);
		System.out.println("Lv."+level+"("+exp+"/100)");
		System.out.println("Hp : " + hp + "/" + maxHp + "\t|\tMp : " + mp + "/" + maxMp);
		System.out.println("att : " + att + "\t|\tdef : " + def);
		System.out.println("-------------인벤토리---------------");
		for(int i = 0; i < items.length; i++){
			if(items[i] != null){
				System.out.println(items[i].itemInfo());
			}
		}
		System.out.println("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■");		
	}
	
	//공격
	void attack(Monster m){
		int damage = att - m.def;
		damage = damage <= 0 ? 1 : damage;
		m.hp = m.hp < damage ? m.hp - m.hp : m.hp - damage;
		System.out.println(name + "가 공격으로 " + m.name + "에게" + damage + "의 데미지를 입혔습니다.");
		System.out.println(m.name + "의 남은 hp : " + m.hp);
	}
	
	//스킬
	void skill(Monster m, Character c){
		int damage = att*2 - m.def;
		damage = damage <= 0 ? 1 : damage;
		m.hp = m.hp < damage ? m.hp - m.hp : m.hp - damage;
		c.mp = c.mp - 10;
		
		System.out.println("스킬을 사용합니다 [무형참]");
		System.out.println(name + "가 공격으로 " + m.name + "에게" + damage + "의 데미지를 입혔습니다.");
		System.out.println(m.name + "의 남은 hp : " + m.hp);
	}
	
	//경험치 업
	void getExp(int exp){
		System.out.println(exp + "의 경험치를 획득하였습니다.");
		this.exp += exp;
		while(100 <= this.exp){
			levelUp();
			this.exp -= 100;
		}
	}
	
	//렙업
	void levelUp(){
		level++;
		maxHp += 10;
		maxMp += 5;
		att += 2;
		def += 2;
		hp = maxHp;
		mp = maxMp;
		System.out.println("***Level Up***");
	}
	
	void getItem(Item item){
		System.out.println("■■■■■■아이템 획득■■■■■■■■");
		System.out.println(item.name + "을 획득하였습니다.");
		System.out.println("■■■■■■■■■■■■■■■■■■■■■");
		for(int i = 0; i < items.length; i++){
			if(items[i] == null){
				items[i] = item;
				break;
			}
		}
		maxHp += item.hp;
		maxMp += item.mp;
		att += item.att;
		def += item.def;
	}
}




