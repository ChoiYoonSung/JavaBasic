...
public class StringClass {
	public static void main(String[] args) {
		 ...
		 * - contains() : 문자열이 특정 문자열을 포함하고 있는지를 반환한다.
...
//		System.out.println(str2.equals(str1));
		...
		String sub2 = str.substring(5,8);	//5번 인덱스부터 8번 인덱스 전까지 잘라서 반환
		System.out.println(sub2);
		
		str = "딸기 수박 참외 메론";
		int index = str.indexOf("메론");
		...
		for (int i = 0; i < menu.length; i++) {
			String fruits = menu[i].substring(0,menu[i].indexOf(" "));
			System.out.println(fruits);
			
			//가격만 잘라서 int타입의 변수에 저장
		...
		//오렌지 메뉴 몇번 인덱스?
		for (int i = 0; i < menu.length; i++) {
			if(menu[i].contains("딸기")){
				System.out.println("딸기는 " + i + "번 인덱스에 있습니다.");
			}
		}
		...
		//trim 사용자가 실수로 문자열에 공백이 들어가는 경우를 방지하기 위해 사용
		//문자열 앞 뒤의 공백 삭제
		str = " 문자열 ";
